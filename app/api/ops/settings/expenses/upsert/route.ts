// app/api/ops/settings/expenses/upsert/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    if (body.id) {
      const upd = await admin.from("ops_expenses").update(payload).eq("id", body.id);
      if (upd.error) return new NextResponse(upd.error.message, { status: 400 });
      return NextResponse.json({ ok: true });
    }

    const ins = await admin.from("ops_expenses").insert(payload);
