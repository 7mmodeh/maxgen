// app/api/ops/settings/expenses/mark-paid/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    if (!body.paid_at?.trim())
      return new NextResponse("paid_at required", { status: 400 });

    const upd = await admin
      .from("ops_expenses")
      .update({ paid_at: body.paid_at, status: body.status })
      .eq("id", body.id);
